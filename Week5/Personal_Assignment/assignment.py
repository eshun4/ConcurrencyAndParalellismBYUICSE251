...
class Factory(threading.Thread):
    ...
    def __init__(self,vehicle_queue, empty_queue, full_queue, queue_statistics, factory_statistics, factory_number, dShip_num, lock, barrier):
        ...
        self._queue_statistics = queue_statistics
        self._barrier = barrier
        self._factory_statistics =factory_statistics
        self._factory_number = factory_number
        self._dShip_num = dShip_num
        self._lock = lock
        super().__init__()
    ...
    def run(self):
        ...
        for i in range(self.cars_to_produce ):
            ...
            self._factory_statistics[self._factory_number] += 1
        ...
        for i in range(self._dShip_num):
            self._empty_queue.acquire()
            self._vehicle_queue.put("NO CARS AVAILABLE")
            self._full_queue.release()
...
class Dealer(threading.Thread):
    ...
    def __init__(self, vehicle_queue, empty_queue, full_queue, queue_statistics,dealership_number, dealer_statistics):
        super().__init__()
        self._vehicle_queue = vehicle_queue
        self._empty_queue = empty_queue
        self._full_queue = full_queue
        self._queue_statistics = queue_statistics
        self._dealership_number = dealership_number
        self._dealer_statistics = dealer_statistics
    ...
    def run(self):
        while True:
            ...
            car = self._vehicle_queue.get()
            if car == "NO CARS AVAILABLE":
                break
            self._dealer_statistics[self._dealership_number] += 1
...
def run_production(factory_count, dealer_count):
    ...
    emp_queue = threading.Semaphore(MAX_QUEUE_SIZE)
    full_queue = threading.Semaphore(0)
    # TODO Create queue
    car_queue = Queue251()
    queue_statistics = list([0] * MAX_QUEUE_SIZE)
    # TODO Create lock(s)
    lock = threading.Lock()
    ...
    facts = [Factory(vehicle_queue=car_queue,empty_queue=emp_queue,barrier=barrier, 
                     full_queue=full_queue, factory_number=factory_numbers,
                     dShip_num=dealer_count,
                     factory_statistics=factory_stats,
                     lock=lock,queue_statistics=queue_statistics) for factory_numbers in range(factory_count)]
    # TODO create your dealerships
    dealerships = [Dealer(vehicle_queue=car_queue,
                          empty_queue=emp_queue,
                          full_queue=full_queue,
                          queue_statistics=queue_statistics, dealer_statistics=dealer_stats, dealership_number=dealership_number)
                   for dealership_number in range(dealer_count)]
    log.start_timer()

    # TODO Start all dealerships
    [d.start() for d in dealerships]
    ...
    [d.join() for d in dealerships]
    [f.join() for f in facts]
